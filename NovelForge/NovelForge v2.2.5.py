...
class DocxHandler:
    ...
    def format_check(self, chapter_num: int, content: str) -> Tuple[bool, list]:
        ...
        for i, p in enumerate(paragraphs[1:], 2):
            p_len = len(p.replace(" ", ""))
            if p_len < 50:
                issues.append(f"第{i}段过短：{p_len}字（要求50-250字）")
            elif p_len > 250:
                issues.append(f"第{i}段过长：{p_len}字（要求50-250字）")
...
class NovelForge:
    ...
    def _write_and_save_chapter(self, chapter_num: int) -> bool:
        ...
        # 元数据截断检测：metadata 为 None 表示输出被截断
        if metadata is None:
            print(f"[截断] 第{chapter_num}章输出被截断（缺少元数据），触发重写")
        ...
        if metadata is None:
            passed = False
            if "截断：缺少元数据，输出可能被token限制截断" not in issues:
        ...
        while not passed and retry_count < self.config.chapter_retry_max:
            ...
            if content:
                # 重写后再次检测截断
                if metadata is None:
                    print(f"[截断] 第{chapter_num}章重写后仍被截断")
                    passed = False
                    issues = ["截断：缺少元数据，输出可能被token限制截断"]
                    severity = "hard"
                else:
                    passed, issues, severity = self._call_ai_review(
                        content, clue_ledger, recent_summaries, chapter_num
    ...
    def _call_ai_review(
        ...
        all_local_issues = hard_issues + warning_issues

        if hard_issues:
            return False, all_local_issues, "hard"
        ...
        try:
            ...
            if review_data:
                ...
                all_issues = all_local_issues + ai_issues
                return passed, all_issues, severity
            else:
                local_severity = "warning" if warning_issues else "none"
                return len(hard_issues) == 0, all_local_issues, local_severity
...
def main():
    print("=" * 60)
    print("  NovelForge v2.2.5 - 长篇小说全自动写作系统")
    print("=" * 60)
