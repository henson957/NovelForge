...
#!/usr/bin/env python3
"""
NovelForge v2.3.1 - 长篇小说全自动写作系统

融合 v2.2.5 流程框架 + v2.2.2 质量保障体系 + InkOS 写作方法论。

变更日志：
  v2.3.1 - 修复 v2.3.0 已知 bug，合并双版本修复
         - [崩溃] from __future__ import annotations 修复 AuditSystem 前向引用
         - [崩溃] AIInterface 构造时校验 api_key/base_url/model，空值直接报错
         - [崩溃] _read_multiline 读 docx 加 try/except，路径不存在不崩溃
         - [崩溃] _start_resume 进度解析加 try/except，格式异常回退第1章
         - [崩溃] main() 初始化 NovelForge 加 try/except，配置错误友好提示
         - [逻辑] 正则 $$thinking$$ → \$$thinking\$$ 正确转义
         - [逻辑] save_chapter 改为 tmp + os.replace 真正原子写入
         - [逻辑] save_chapter / format_check 用正则判断标题，不再假设第一段是标题
         - [逻辑] _write_file 按行拆分写入 docx，不再塞成单段落
         - [逻辑] _save_original 按行拆分写入 docx
         - [逻辑] save_review_record 加原子写入（tmp + replace）
         - [逻辑] _split_content_and_metadata 改用正则匹配元数据，容忍空格差异
         - [逻辑] recent_summary_window 3 → 8，适配 pacing/重复检查的 5 章窗口
         - [逻辑] check_warnings 高频词阈值 c>3 → c>8，加 stop words 避免误报
         - [逻辑] _start_import 逐章分析加 try/except，单章失败不中断导入
  v2.3.0 - 融合 v2.2.2 的 InkOS 写作方法论、上下文精简、AI 痕迹检测、
           18 维度审计、信息边界、资源追踪、情感弧线、崩溃保护
         - 新增 ContextBuilder / AITellDetector / PostWriteValidator / AuditSystem
         - 新增 character_matrix / resource_ledger / emotional_arcs 追踪
         - 新增 atomic_write / InkOS 方法论 / 审计→修订循环
         - 保留 v2.2.5 的材料确认、4 轮异常处理、阶段审核、终审、截断检测、导入模式
  v2.2.5 - 基础流程框架
  v2.2.2 - InkOS 方法论、AI 检测、审计系统
...
class AuditSystem:
    ...
    def _check_resources(self, content: str) -> List[Dict]:
        ...
        if not ledger:
            return []
        issues = []
        for m in re.finditer(r'-\s*(\S{2,8}?)\s*[|｜]\s*(.+)', ledger):
            name = m.group(1)
            description = m.group(2).strip()
            numbers = re.findall(r'(\d+)', description)
            if not numbers or name not in content:
                continue
            expected = int(numbers[-1])
            actual_m = re.search(re.escape(name) + r'[^，。！？\n]{0,10}?(\d+)', content)
            if actual_m:
                actual = int(actual_m.group(1))
                if actual != expected:
                    issues.append({"severity": "warning", "category": "资源连续性",
                        "description": f"'{name}'账本记录{expected}，正文{actual}",
                        "suggestion": "核实资源数量"})
        return issues
    ...
    def _check_numerical_consistency(self) -> List[Dict]:
        ...
        for m in re.finditer(r'-\s*(\S{2,8}?)\s*[|｜]\s*(.+)', ledger):
            name = m.group(1)
            description = m.group(2).strip()
            if re.search(r'[变为剩余扣减结余]\s*[-－]\d+', description):
                neg = re.search(r'[-－](\d+)', description)
                if neg:
                    issues.append({"severity": "warning", "category": "数值体系",
                        "description": f"'{name}'出现负值：-{neg.group(1)}",
                        "suggestion": "检查扣减逻辑"})
        return issues
...
class AIInterface:
    ...
    def __init__(self, config: Config):
        if not config.ai_api_key or not config.ai_base_url or not config.ai_model:
            raise ValueError("AI 配置不完整：API Key、Base URL 和 Model 不能为空")
        self.config = config
...
class MemorySystem:
    ...
    def update_clue_ledger(self, clue_changes: list):
        ...
        if marker in current:
            updated = current.replace(marker, f"{changes_text}\n\n{marker}")
        else:
            updated = current + f"\n\n{changes_text}\n\n{marker} {now}"
...
class DocxHandler:
    ...
    def save_chapter(self, chapter_num: int, content: str) -> str:
        ...
        has_title = bool(paragraphs and self.CHAPTER_TITLE_RE.match(paragraphs[0]))
    ...
    def format_check(self, chapter_num: int, content: str) -> Tuple[bool, list]:
        ...
        if not paragraphs:
            issues.append("内容为空")
            return False, issues
        has_title = bool(self.CHAPTER_TITLE_RE.match(paragraphs[0]))
        body_paragraphs = paragraphs[1:] if has_title else paragraphs
        ...
        start_index = 2 if has_title else 1
        for i, p in enumerate(body_paragraphs, start_index):
            p_len = len(p.replace(" ", ""))
            if p_len < 50:
...
class ReviewStandards:
    ...
    HIGH_FREQ_STOP = {"一个", "这个", "那个", "他们", "她们", "我们", "你们",
                      "什么", "怎么", "可以", "没有", "但是", "然后", "因为",
                      "所以", "如果", "只是", "已经", "正在", "不是", "他的",
                      "她的", "我的", "你的", "就是", "还是", "这些", "那些"}
    ...
    def check_warnings(content: str, transition_interval: int = 1000) -> list:
        ...
        for w, c in word_count.items():
            if c > 8 and len(w) <= 2 and w not in ReviewStandards.HIGH_FREQ_STOP:
...
class NovelForge:
    ...
    def _start_new(self, story_bible: str, chapter_outlines: str):
        print(f"[NovelForge] v2.3.1 项目：{self.config.project_name}")
    ...
    def _start_resume(self):
        ...
        try:
            chapter_str = progress.get("下一章", "第1章").replace("第", "").replace("章", "")
            next_chapter = int(chapter_str) if chapter_str.strip() else 1
        except (ValueError, AttributeError):
            print("[NovelForge] 进度文件异常，从第1章开始")
            next_chapter = 1
    ...
    def _start_import(self, story_bible: str, chapter_outlines: str,
        ...
        for i in range(1, start_chapter):
            ...
            try:
                ...
                print(f"[NovelForge] 第{i}章分析失败：{e}，跳过该章记忆更新")
            if analysis_data:
    ...
    def _write_and_save_chapter(self, chapter_num: int) -> bool:
        ...
        if audit_issues:
            ...
            if fixable:
                content = self._audit_revise_loop(
                    chapter_num, content, chapter_outline, context, fixable)
        else:
            print(f"  [审计] 通过")
    ...
    def _call_ai_write(self, chapter_num: int, chapter_outline: str,
                       context: Dict[str, str], feedback: str = None
    ...
    def _call_ai_review(self, content: str, clue_ledger: str,
        ...
        warning_issues = ReviewStandards.check_warnings(
            content, self.config.transition_word_interval)
    ...
    def _split_content_and_metadata(self, response: str) -> Tuple[str, Optional[dict]]:
        meta_match = re.search(
            r'---\s*元数据\s*---(.*?)---\s*元数据结束\s*---', response, re.DOTALL)
        if meta_match:
            content = response[:meta_match.start()].strip()
...
def main():
    ...
    print("=" * 60)
    print("  NovelForge v2.3.1 - 长篇小说全自动写作系统")
    print("=" * 60)
...
def _run_from_args():
    ...
    config = Config()
    config.ai_base_url = args.get("base-url", "")
    config.ai_api_key = args.get("api-key", "")
    config.ai_model = args.get("model", "")
    config.project_name = args.get("name", "我的小说")
    config.total_chapters = int(args.get("chapters", "500"))
    config.words_per_chapter = int(args.get("words", "2500"))
    config.writing_style = args.get("style", "")
    config.special_requirements = args.get("special", "")
    config.review_standards = args.get("review", "")
    config.skip_material_check = True

    bible_path = args.get("bible", "")
    outlines_path = args.get("outlines", "")
    ...
    if bible_path:
        if bible_path.endswith(".docx"):
            doc = Document(bible_path)
            story_bible = "\n".join(p.text for p in doc.paragraphs)
        else:
    ...
    if outlines_path:
        if outlines_path.endswith(".docx"):
            doc = Document(outlines_path)
            chapter_outlines = "\n".join(p.text for p in doc.paragraphs)
        else:
    ...
    if mode == "new":
        ...
        if folder and os.path.isdir(folder):
            for fname in sorted(os.listdir(folder)):
                ...
                if num_match and os.path.isfile(fpath):
                    ch_num = int(num_match.group(1))
                    if fpath.endswith(".docx"):
                        doc = Document(fpath)
                        existing_chapters[ch_num] = "\n".join(p.text for p in doc.paragraphs)
                    else:
                        with open(fpath, "r", encoding="utf-8") as f:
                            existing_chapters[ch_num] = f.read()
        forge.start("import", story_bible=story_bible,
                    chapter_outlines=chapter_outlines,
                    existing_chapters=existing_chapters,
                    start_chapter=start)
