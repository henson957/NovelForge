...
class AIInterface:
    ...
    def generate(self, system_prompt: str, user_prompt: str) -> str:
        ...
        return self._clean_think_tags(response.choices[0].message.content)
...
class MemorySystem:
    ...
    def update_global_memory(self, updates: str):
        ...
        update_lines = [l.strip() for l in updates.strip().split("\n") if l.strip()]

        # 对于角色状态类更新（包含冒号的行），用新值替换旧值
        merged = {}
        for line in current_lines:
            ...
            if key:
                ...
                merged[f"_extra_{len(merged)}"] = line

        for line in update_lines:
            key = line.split("：")[0].strip() if "：" in line else None
            if key:
                merged[key] = line  # 覆盖旧值
            else:
                merged[f"_extra_{len(merged)}"] = line

        result = "\n".join(merged.values())
        # 限制记忆大小，超出时保留后半部分（较新的内容）
        max_chars = 8000
        if len(result) > max_chars:
            result = result[-max_chars:]
        self._write_file("全局记忆.docx", result)
...
class DocxHandler:
    ...
    def format_check(self, chapter_num: int, content: str) -> Tuple[bool, list]:
        ...
        for i, p in enumerate(paragraphs[1:], 2):
            p_len = len(p.replace(" ", ""))
            if p_len < 10:
                issues.append(f"第{i}段过短：{p_len}字（要求10-250字）")
            elif p_len > 250:
                issues.append(f"第{i}段过长：{p_len}字（要求10-250字）")
...
class PromptBuilder:
    ...
    def build_writing_prompt(
        ...
        system = (
            '你是一个小说写作AI。根据提供的材料写一章正文。\n\n'
            '【写作要求】\n'
            f'1. 创作必须严格控制在{self.config.words_per_chapter - 300}-{self.config.words_per_chapter + 300}字之间，不允许偏离\n'
            '2. 不得与记忆系统中的任何信息矛盾\n'
            '3. 自然收尾，留白或场景转换，禁止总结性段落和AI式结尾\n'
            '4. 第一行必须是章节标题，格式为「第X章 标题」\n'
            f'{style_requirement}'
            f'{special_requirement}'
            f'{custom_review}'
            '\n'
            '【违禁规则 - 硬性】\n'
            '- 正文中禁止出现「第X章」「chapter X」等章节号（标题行除外）\n'
            '- 禁止破折号「——」\n'
            '- 禁止分析报告术语（核心动机、信息落差、认知共鸣、锚定效应、叙事张力、情感锚点、角色弧光、戏剧冲突）\n'
            '- 禁止未设定的人物/地名\n'
            '- 禁止真实地名（除非大纲明确）\n'
            '- 禁止违背现实逻辑的情节\n'
    ...
    def build_review_prompt(
        ...
        system = (
            ...
            '5. 是否存在硬性违禁项\n'
            '6. 是否存在警告项\n'
            f'7. 字数必须在2200-2800字范围内，超出或不足均为hard错误\n'
            f'{custom_review}'
            '\n'
            '【severity严重程度说明】\n'
            '- hard：存在硬性错误，必须重写\n'
...
class NovelForge:
    ...
    def _start_new(self, story_bible: str, chapter_outlines: str):
        ...
        if not self.config.skip_material_check:
            ...
            if not passed:
                ...
                print("或者设置 skip_material_check=True 跳过确认直接开始写作。")
                return
        else:
        ...
        if memory_data:
            if "global_memory" in memory_data:
                ...
                print("[NovelForge] 全局记忆已从故事圣经提取")
            if "clue_ledger" in memory_data:
                self.memory._write_file("线索台账.docx", memory_data["clue_ledger"])
                print("[NovelForge] 线索台账已从故事圣经提取")
        else:
            # AI 解析失败，手动从故事圣经截取关键信息作为初始记忆
            print("[NovelForge] 警告：AI记忆初始化解析失败，使用故事圣经原文作为全局记忆")
            self.memory._write_file("全局记忆.docx", story_bible[:3000])
            self.memory._write_file("线索台账.docx", "（等待写作过程中逐步填充）")
    ...
    def _write_and_save_chapter(self, chapter_num: int) -> bool:
        ...
        # 保存审核记录
        review_summary = f"第{chapter_num}章审核\n通过：{passed}\n严重程度：{severity}\n"
        if issues:
        ...
        self.docx.save_review_record(f"第{chapter_num:03d}章_审核.docx", review_summary)

        # 只有当元数据实际有内容时才更新记忆系统
        has_metadata = (
            metadata.get("summary") or
            metadata.get("clue_changes") or
            metadata.get("suggested_updates")
        )
        if has_metadata:
            if metadata.get("summary"):
                self.memory.update_recent_summaries(chapter_num, metadata["summary"])
            if metadata.get("clue_changes"):
                self.memory.update_clue_ledger(metadata["clue_changes"])
    ...
    def _rewrite_chapter(self, chapter_num: int) -> bool:
        ...
        print(f"[重写] 调用AI重写第{chapter_num}章...")
        content, metadata = self._call_ai_write(
            chapter_num, global_memory, clue_ledger,
            recent_summaries, chapter_outline
        )
    ...
    def _split_content_and_metadata(self, response: str) -> Tuple[str, dict]:
        ...
        if meta_match:
            ...
            meta_text = response[meta_start:].strip()
            return self._parse_meta_text(content, meta_text, metadata)

        # 2. 退而求其次：找单独出现的"摘要："（前面有分隔线）
        ...
        # 3. 再退：直接找"摘要："前面最近的分隔线
        idx = response.find("摘要：")
        if idx < 0:
            idx = response.find("摘要:")
        if idx > 0:
            before = response[:idx]
            dash_pos = before.rfind("---")
            if dash_pos >= 0 and dash_pos > len(before) - 100:
                meta_match_start = dash_pos
            else:
                nl_pos = response.rfind("\n", 0, idx)
                if nl_pos >= 0:
                    meta_match_start = nl_pos + 1
                else:
                    meta_match_start = idx
            content = response[:meta_match_start].strip()
            meta_text = response[meta_match_start:].strip()
            return self._parse_meta_text(content, meta_text, metadata)

        # 4. 最后兜底：找"线索变化："或"建议更新："（可能AI跳过了摘要行）
        for marker in ["线索变化：", "线索变化:", "建议更新：", "建议更新:"]:
        ...
        # 5. 完全没找到元数据标记，返回原文（不做任何处理）
        return response.strip(), metadata

    def _parse_meta_text(self, content: str, meta_text: str, metadata: dict) -> Tuple[str, dict]:
        """解析元数据文本"""
        ...
        for line in meta_text.split("\n"):
            ...
            if not line:
                continue
            # 跳过标记行
            if re.match(r'^[-]+\s*元数据', line):
                continue
            if re.match(r'^[-]+\s*元数据结束', line):
                break

            if re.match(r'^摘要[：:]', line):
                metadata["summary"] = re.sub(r'^摘要[：:]\s*', '', line).strip()
                current_section = "summary"
            elif re.match(r'^线索变化[：:]', line):
                current_section = "clues"
            elif re.match(r'^建议更新[：:]', line):
                value = re.sub(r'^建议更新[：:]\s*', '', line).strip()
...
def main():
    print("=" * 60)
    print("  NovelForge v2.1.2 - 长篇小说全自动写作系统")
    print("=" * 60)
