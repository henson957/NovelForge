...
v2.0.5 变更（相对 v2.0.4）：
1. 新增 OpenClawConfigReader 类，自动从 ~/.openclaw/openclaw.json 读取 API 配置
2. AIInterface 自动检测 OpenClaw 配置，用户无需手动填写 API 信息
3. 保留手动配置作为 fallback
4. 命令行不再要求输入 API 信息
...
class OpenClawConfigReader:
    ...
    def read() -> dict:
        ...
        # 提取 agent 配置中的 model 和 provider
...
class NovelForge:
    ...
    v2.0.5 架构说明：
    AI 负责写作和审核，Python 代码负责控制流程。
    Python 控制：什么时候写、什么时候审、什么时候保存、什么时候重写、什么时候阶段审核、什么时候终审。
    AI 负责：确认材料、写章节正文、审核章节。
    每次 AI 调用通过 AIInterface.generate() 发送独立 HTTP 请求，无状态、无上下文关联。
    所有流程控制均由 Python 代码保证执行，不会遗漏任何步骤。
    阶段审核和终审发现问题后立即修复，修复完毕才继续。
    API 配置自动从 OpenClaw 配置文件读取，用户无需手动填写。
    """
    ...
    def start(self, mode: str, **kwargs):
        ...
        mode:
            "new"     - 全新写作（开始写xxxx小说）
            "resume"  - 断点续写（继续写xxxx小说）
            "import"  - 导入续写（从第X章开始续写xxxx小说）
        """
        if mode == "new":
...
def main():
    """命令行交互入口"""
    print("=" * 60)
    print("  NovelForge v2.0.5 - 长篇小说全自动写作系统")
    print("=" * 60)

    config = Config()

    # 自动从 OpenClaw 读取 API 配置
